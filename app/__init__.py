...
def create_app(args):
    ...
    def pubmed_api(pmids):
        ...
        result_dicts = [app.config["model"].annotate_pmid(pmid=pmid) for pmid in pmids]
    ...
    def plain_api():
        ...
        # annotate input
        result_dict = app.config["model"].annotate_text(text=sample_text)
    ...
    def send_data():
        ...
        if req_type == "text":
            ...
            _code, parse_res, keys_in_dict = app.config["r_parser"].parse_result(result_dict, draw_keys, result_id="text")
            ...
            res_items.append({
                ...
                'keys': {k: app.config["r_parser"].entity_type_dict[k] for k in keys_in_dict}
            })
            ...
            if not args.front_dev:
                result_dicts = [app.config["model"].annotate_pmid(pmid=pmid) for pmid in _pmids]
            else:
            ...
            for _pmid, result_dict in pmid2result_dicts.items():
                _code, parse_res, keys_in_dict = app.config["r_parser"].parse_result(result_dict, draw_keys, result_id=_pmid)
                ...
                legend_items = {k: app.config["r_parser"].entity_type_dict[k] for k in keys_in_dict}
